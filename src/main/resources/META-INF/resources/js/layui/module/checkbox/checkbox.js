...
layui.define('jquery', function(exports){
	...
	CheckBox.prototype.checkbox = function(elem,children){
		...
		layui.each(nodes, function(index, item){
			var tipsContent = tips.render?(item.tipsContent?item.tipsContent:''):item.type;
			var li = $(['<li class="block'+(item.on?' on':'')+'" value="'+item.name+ '" '+(tips.show?'onmouseover="layui.layer.tips(\''+tipsContent+'\',this,{tips:3})" onmouseout="layui.layer.closeAll(\'tips\');"':'')+'>'+item.name,'<i class="choice"><i class="triangle"></i><i class="right layui-icon layui-icon-ok"></i></i>'+(showDel?'<i class="del"><i class="layui-icon layui-icon-delete"></i></i>':'')+'<span class="hide">'+(item.on?'<input type="hidden" name="'+item.name+'" value="'+item.type+'">':'')+'</span></li>'].join(''));
			elem.append(li);
			//触发点击节点回调
