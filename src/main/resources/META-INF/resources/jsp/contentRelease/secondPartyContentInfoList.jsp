                    ...
                    <tr>
                        ...
                        <td data-label="操作" class="operation_btn">
                        	<c:if test="${info.publisher_id == user_name }"><!-- 新闻发布人和登录人相同的可以操作 -->
	                        	<c:if test="${(info.approve_state == 4 || info.approve_state == 6)&& empty info.first_approve_id}"><!-- 组织部驳回且没有一审审核人或者已撤回的新闻可以编辑或者删除 -->
									<portlet:renderURL var="addInfoUrl">
										<portlet:param name="mvcRenderCommandName" value="/info/addInfo" />
...
<%-- 									<a onclick="update('${info.resources_id }');"> --%>
								...
								<c:if test="${info.approve_state == 1 && empty info.first_approve_id }"><!-- 组织部审核中且没有一审审核人的新闻可以撤回 -->
