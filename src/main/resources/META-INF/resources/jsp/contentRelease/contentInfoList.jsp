			...
			<div class="form-group">
                ...
                <div class="col-sm-6 col-xs-12">
                    ...
                    <div class="col-sm-10 col-xs-9">
                        <input type="text" name="date" id="labCheckDate"  value="${date }"
                        onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})"   class="form-control start_date">
                    </div>
                </div>
                <div class="col-sm-6 col-xs-12">
                    <span class="col-sm-2 col-xs-3 control-label">审核状态</span>
                    <div class="col-sm-10 col-xs-9">
						<select class="form-control"  name="<portlet:namespace/>approve_state" id="approve_state">
							<c:if test="${empty approve_state}">
								<option value="" selected="selected"></option>
								<c:forEach var="states" items="${states}" >
									<option value="${states.id }">${states.mark }</option>
								</c:forEach> 
							</c:if>
							<c:if test="${not empty approve_state}">
								<option value=""></option>
								<c:forEach var="states" items="${states}" >
									<c:if test="${states.id == approve_state }">
										<option value="${states.id }" selected="selected">${states.mark }</option>
									</c:if>
									<c:if test="${states.id != approve_state }">
										<option value="${states.id }">${states.mark }</option>
									</c:if>
            ...
            <table class="content_table">
                ...
                <tbody class="table_info">
                    ...
                    <tr>
                        ...
                        <td data-label="二级党组织审核人">${info.first_approve_id }</td>
                        <td data-label="二级党组织驳回原因">${info.first_dismissal }</td>
                        <td data-label="组织部审核人">${info.second_approve_id }</td>
                        <td data-label="组织部驳回原因">${info.second_dismissal }</td>
                        <td data-label="操作" class="operation_btn">
                        	<c:if test="${info.publisher_id == user_name }"><!-- 新闻发布人和登录人相同的可以操作 -->
	                        	<c:if test="${info.approve_state == 2 || info.approve_state == 5 || info.approve_state == 4}"><!-- 二级党组织驳回或者已撤回的新闻可以编辑或者删除 -->
									<portlet:renderURL var="addInfoUrl">
										<portlet:param name="mvcRenderCommandName" value="/info/addInfo" />
...
<%-- 									<a onclick="update('${info.resources_id }');"> --%>
								...
								<c:if test="${info.approve_state == 0 }"><!-- 二级党组织审核中的新闻可以撤回 -->
									<portlet:renderURL var="recallInfoUrl">
										<portlet:param name="mvcRenderCommandName" value="/info/recallInfoUrl" />
										<portlet:param name="resourceId" value="${info.resources_id}" />
										<portlet:param name="contentPortletKey" value="${contentPortletKey }" />
									</portlet:renderURL>
<%-- 									<a href="${recallInfoUrl }&resourceId=${info.resources_id }&contentPortletKey=${contentPortletKey }"> --%>
									<a onclick="recall('${info.resources_id }');">
										<button type="button" class="btn btn-default td_assign_btn">
											撤回&nbsp;
										</button>
									</a>
