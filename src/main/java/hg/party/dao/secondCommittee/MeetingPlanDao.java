...
//   }
    ...
    public Map<String, Object> queryPassedMeetingsByOrgId(String orgId, String search, int page) {
        String sql = "select * from hg_party_meeting_plan_info where" +
                " organization_id = ? and task_status >= '4' ";
        if (!StringUtils.isEmpty(search)) {
            search = "%" + search + "%";
            sql = sql + " and (meeting_type like ? or meeting_theme like ?)";
        }
        ...
        if (!StringUtils.isEmpty(search)) {
            list = this.jdbcTemplate.queryForList(sql1, orgId, search, search);
            count = this.jdbcTemplate.queryForList(sql, orgId, search, search);
        }else {
            list = this.jdbcTemplate.queryForList(sql1, orgId);
            count = this.jdbcTemplate.queryForList(sql, orgId);
        }
        int total = count.size();
        if (total % 8 == 0) {
    ...
    public Map<String, Object> queryInformMeetingsByOrgId(String orgId, String search, int page) {
        String sql = "select plan.*,note.id as noteId from hg_party_meeting_plan_info plan " +
...
//	   logger.info("sql :" + sql);
    ...
    public List<Map<String, Object>> queryMeetingPlanByMeetingIdBranch(String meetingId) {
        String sql = "SELECT m.*,b.org_id AS sid,b.org_name AS bname,s.org_name AS sname,s.org_id,s.org_type" +
                " FROM public.hg_party_org AS b ,public.hg_party_meeting_plan_info AS m ,public.hg_party_org AS s ,public.hg_party_org AS p WHERE m.meeting_id = '" + meetingId + "' AND m.organization_id = b.org_id and b.historic is false AND m.organization_id = p.org_id AND p.org_parent = s.org_id";
//	   logger.info("sql :" + sql);
        return this.jdbcTemplate.queryForList(sql);
    }

    public List<Map<String, Object>> meetingParticipants(String meetingId) {
        String sql = " SELECT m.participant_id , p.member_name FROM hg_party_meeting_member_info AS m " +
                "inner JOIN hg_party_member p on m.participant_id = p.member_identity and p.historic = false" +
                " WHERE m.meeting_id = ?";
//	   logger.info("sql :" + sql);
        return this.jdbcTemplate.queryForList(sql, meetingId);
    }
...
//	   logger.info("sql :" + sql);
    ...
    public void informParty(String meetingId, String participant_id) {
        ...
        this.jdbcTemplate.update(sql, meetingId, participant_id);
    }
