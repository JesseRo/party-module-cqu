...
@Component(
	...
	property = {
		...
		"mvc.command.name=/party/uploadExperience"
    },
    service = MVCRenderCommand.class
)
public class ExperienceRenderCommand implements MVCRenderCommand {
	...
	public String render(RenderRequest renderRequest, RenderResponse renderResponse) throws PortletException {
		...
		//心得内容
		String heart = "";
		if(partyMeetingPlanInfoService.experience(meetingId,userId).size() > 0){
			heart = partyMeetingPlanInfoService.experience(meetingId,userId).get(0).get("experience_content").toString();
		}
		heart = HtmlUtil.escape(heart);
		
		//设置独占表单
		String formId = UUID.randomUUID().toString();
		SessionManager.setAttribute(renderRequest.getRequestedSessionId(), "formId-informParty", formId);
		renderRequest.setAttribute("formId", formId);
		
		renderRequest.setAttribute("heart", heart);
		...
		return "/jsp/toDoList/experience.jsp";
	}
