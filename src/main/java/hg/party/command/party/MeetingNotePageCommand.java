...
@Component(
		...
		property = {
				...
				"mvc.command.name=/org/meetingNote/page"
		},
		service = MVCResourceCommand.class
	)

public class MeetingNotePageCommand implements MVCResourceCommand {
	@Reference
		...
		try {
			...
			if(orgId!=null && !StringUtils.isEmpty(String.valueOf(orgId))){
				if (StringUtils.isEmpty(keyword)){
					data = partyMeetingNoteService.meetingNotePageAndSearch(page, size,String.valueOf(orgId),null);
				}else {
					data = partyMeetingNoteService.meetingNotePageAndSearch(page, size, String.valueOf(orgId),keyword);
				}
			}
			Gson gson = new Gson();
