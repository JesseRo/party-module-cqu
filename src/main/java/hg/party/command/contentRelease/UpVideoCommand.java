...
@Component(
	...
	property = {
		...
		"mvc.command.name=/ueditor/uploadVideo"
    },
    service = MVCResourceCommand.class
)
public class UpVideoCommand implements MVCResourceCommand {
	Logger logger = Logger.getLogger(UpVideoCommand.class);
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse){
		...
		try {
			logger.info("UpVideoCommand serveResource...........");
...
//			Long  sizInBytes = uploadPortletRequest.getSize(fileName);
			...
			if(uploadFiles != null){
				for(File file : uploadFiles){
					...
					//File.separator
					String videoUrl = "/" + sourceFileName;
					if("" != suffix){
						videoUrl = "/" +"uploadVideo" + "/" + UUID.randomUUID().toString() + "." + suffix;
					}
					
					File filePath = new File(folder.getAbsolutePath() + videoUrl);
					//保存文件到物理路径
